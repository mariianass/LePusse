...
public class ClienteDAOTest {
    ...
    void guardarClienteValido() throws PersistenciaException {
        ...
        assertEquals("6441112222", resultado.getTelefono());
    }
    ...
    void testBuscarPorIdExistente() throws PersistenciaException {
        ...
        assertEquals("Isaac", encontrado.getNombre());
    }
    ...
    void testEditarCliente() throws PersistenciaException {
        ...
        guardado.setCorreoElectronico("josejoaquin@test.com");

        Cliente actualizado = clienteDAO.editar(guardado);

        assertNotNull(actualizado);
        assertEquals("Jose Joaquin", actualizado.getNombre());
        assertEquals("josejoaquin@test.com", actualizado.getCorreoElectronico());
    ...
    void buscarPorFiltroNombreParcial() throws PersistenciaException {
        ...
        cliente.setCorreoElectronico("martina@test.com");
    ...
    @Test
    void obtenerTodosDebeRegresarClientesGuardados() throws PersistenciaException {
        Cliente cliente = new Cliente();
        cliente.setNombre("Maria");
        cliente.setApellidoPaterno("Lopez");
        cliente.setApellidoMaterno("Garcia");
        cliente.setTelefono("6440001122");
        cliente.setCorreoElectronico("maria@test.com");

        Cliente guardado = clienteDAO.guardar(cliente);
        registrarIdSiExiste(guardado);

        List<Cliente> resultados = clienteDAO.obtenerTodos();

        assertNotNull(resultados);
        assertTrue(
                resultados.stream().anyMatch(c -> c.getId().equals(guardado.getId())),
                "El cliente guardado debería aparecer en obtenerTodos()"
        );
    }
}